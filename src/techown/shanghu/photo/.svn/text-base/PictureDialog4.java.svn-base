...
public class PictureDialog4 {
	...
	public PictureDialog4(Context context,Bitmap bit,String picname){
...
//		preview.setRotation(270);
	...
	class deleteBtnListener implements OnClickListener{
		public void onClick(View v) {
			...
			((GongDanTakePhotoActivity)context).handlerphoto.sendMessage(msg);
			dlg.dismiss();
		}
	}
	class rePhotoBtnListener implements OnClickListener{
		public void onClick(View v) {
			...
			((GongDanTakePhotoActivity)context).handlerphoto.sendMessage(msg);
			System.out.println("photoid->"+temppicname);
//			if (TuanDuiShangHuActivity.device == 0) {
//				GongDanTakePhotoActivity.request =4;
...
//				((GongDanTakePhotoActivity)context).startActivityForResult(i, 4);
