...
//		getLogic();
		...
		while (cursor.moveToNext()) {
			try {
				...
				techown.shanghu.https.Log.Instance().WriteLog("团队HeYueQianDing_441:"+e.getMessage());
			}finally{
				if(cursor != null){
					cursor.close();
				}if(db != null){
    	...
    	techown.shanghu.https.Log.Instance().WriteLog("HeYueQianDing_441类："+e.getMessage());
    ...
    public void saveData() {
		...
		DB.upload(HeYueQianDing_441.this, clomname, clomstr, TuanDuiShangHuActivity.num);
    		...
    		techown.shanghu.https.Log.Instance().WriteLog("HeYueQianDing_441类saveData()"+e.getMessage());
    	}
	}
...
//					saveLogic();
				...
				techown.shanghu.https.Log.Instance().WriteLog("HeYueQianDing_441类按钮点击："+e.getMessage());
