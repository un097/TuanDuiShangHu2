...
public class HeYueQianDing_3 extends Activity {
		...
		try {
			...
			while (cursor.moveToNext()) {
				try {
					...
					techown.shanghu.https.Log.Instance().WriteLog(
							"团队HeYueQianDing_3:" + e.getMessage());
				} finally {
					if (cursor != null) {
						cursor.close();
					}
			...
			h1leout72.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					if (TuanDuiShangHuActivity.agencycityname.length != 0
						...
						log.dialog(HeYueQianDing_3.this, h1et702, "市",
					...
					try {
						if (!h1et702.getText().toString().equals("")
							...
							if (str1 != null && str1.length > 0) {
								log.dialog(HeYueQianDing_3.this, h1et703, "区",
										str1);
							} else {
								h1et703.setText("无");
							...
							log.Toast(HeYueQianDing_3.this, "请选择城市！");
						}
					} catch (Exception e) {
			...
			linlay[6].setOnTouchListener(new OnTouchListener() {
				public boolean onTouch(View arg0, MotionEvent arg1) {
					switch (arg1.getAction()) {
					case MotionEvent.ACTION_UP:
						log.dialog(HeYueQianDing_3.this, textArray[6], "菜系",
								t_childtyp);
						break;
					default:
						break;
					...
					case MotionEvent.ACTION_UP:
						if (h1et702.getText().toString().length() > 0) {
							...
							if (t_trading1 != null && t_trading1.length > 0) {
								...
								log.Toast(HeYueQianDing_3.this, "该城市没有商圈！");
							}
						} else {
							log.Toast(HeYueQianDing_3.this, "请先选择城市！");
						}
						break;
					default:
						break;
			...
			linlay[15].setOnTouchListener(new OnTouchListener() {
				public boolean onTouch(View arg0, MotionEvent arg1) {
					switch (arg1.getAction()) {
					case MotionEvent.ACTION_UP:
						log.dialog(HeYueQianDing_3.this, textArray[15], "类型",
								province1);
						break;
					default:
						break;
			...
			linlay[16].setOnTouchListener(new OnTouchListener() {
				public boolean onTouch(View arg0, MotionEvent arg1) {
					...
					case MotionEvent.ACTION_UP:
						...
						log.num = 1;
						// lxp
						log.dialog(HeYueQianDing_3.this, textArray[16], "类型",
								province);
						break;
					default:
						break;
			...
			linlay[18].setOnTouchListener(new OnTouchListener() {
				public boolean onTouch(View arg0, MotionEvent arg1) {
					switch (arg1.getAction()) {
					case MotionEvent.ACTION_UP:
						if (textArray[16].getText().toString().equals("是")) {
							log.etDialog(HeYueQianDing_3.this, textArray[18],
									R.id.h2edtext19, "请输入收单费率");
						} else {
							log.Toast(HeYueQianDing_3.this, "请选择是否收单！");
						}
						break;
					default:
						break;
			...
			linlay[19].setOnTouchListener(new OnTouchListener() {
				public boolean onTouch(View arg0, MotionEvent arg1) {
					switch (arg1.getAction()) {
					case MotionEvent.ACTION_UP:
						log.dialog(HeYueQianDing_3.this, textArray[20], "类型",
								bb);
				...
				public boolean onTouch(View arg0, MotionEvent arg1) {
					...
					case MotionEvent.ACTION_UP:
						log.Dialog(HeYueQianDing_3.this, linlay[20],
								textArray[21], R.id.h2edtext21, "请输入白名单编号");
						break;
					default:
						break;
					}
	...
	public void init() {
		try {
			...
			techown.shanghu.https.Log.Instance().WriteLog(
					"HeYueQianDing_3类init（）：" + e.getMessage());
		}
	}
	...
	protected boolean validate() {
		...
		if (textArray[21].getText().toString().length() >= typCode[1].length()) {
			...
			log.Toast(HeYueQianDing_3.this, "白名单编号与商户类型不符！");
			return false;
	...
	public void getdata(Context con) {
		...
		try {
			...
			techown.shanghu.https.Log.Instance().WriteLog(
					"团队：HeYueQianDing_3+getdata出错" + e.getMessage());
		} finally {
	...
	public String queryCityName(String username) {
		...
		try {
			...
			techown.shanghu.https.Log.Instance().WriteLog(
					"团队：HeYueQianDing_3+queryCityName出错" + e.getMessage());
		} finally {
			if (cursor != null) {
				cursor.close();
			}
