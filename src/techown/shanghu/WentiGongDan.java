...
//				mDialog.dismiss();
			...
			case 7:// 获取gps失败
				...
				log.Toast(WentiGongDan.this,"GPS获取失败");
				stoptime = System.currentTimeMillis();
				break;
			case 8:
				if(stoptime==0)
				{
					mGPS.close();
					hideWaitingDialog();
					log.Toast(WentiGongDan.this, "获取GPS超时！");
					stoptime = 0;
				}
				break;
			case 9:// 获取gps失败
...
//        initChView();
    ...
    public void saveData() {
		...
		for (int i = 0; i < clomstr.length; i++) {
			System.out.println("问题上门工单："+clomstr[i]);
		}
		...
		public void onClick(View v) {
			...
			switch(id){
				...
				case R.id.button2:
    				...
    				new AlertDialog.Builder(WentiGongDan.this).setTitle("提示").setMessage("提交成功")
    	...
    	public void run() {
    		...
    		while(true){
	    		...
	    		try{
	    			...
	    			techown.shanghu.https.Log.Instance().WriteLog("WentiGongDan类GpsOverTime"+e.getMessage());
	    		}
    		}
    	}
    ...
    public void init()
    {
		...
		{
			techown.shanghu.https.Log.Instance().WriteLog("WentiGongDan类。。。。"+e.getMessage());
		}	
    }
...
//		}
	...
	techown.shanghu.https.Log.Instance().WriteLog("WentiGongDan类getLocation()"+e.getMessage());
	}
	}
	...
	protected boolean validate() {
		...
		public void save(){
			 try{
					...
					techown.shanghu.https.Log.Instance().WriteLog("WentiGongDan类save（）。。。"+e.getMessage());
				}
		}
		String Dnum;
